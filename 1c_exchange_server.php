...
Отправляет данные через API в opencart
*/
...
function process_checkauth_request()
{
    echo 'success';
    echo PHP_EOL;
    echo 'login';  //доработка
    echo PHP_EOL;
    echo 'password'; //доработка
...
function process_file_request()
{
    ...
    $filename = $_GET['filename'];   //Доработка
...
//Доработка
function get_files_list_from_folder($folder){
    //Т.к. функция scandir возвращает путь к родительскому каталогу .  и ..
    //которые нам не нужны. Удалим их.

    $all_files = scandir($folder);
    $cleened_files_list = array_diff($all_files, array('..', '.'));

    return $cleened_files_list;
...
function send_data_to_opencart_api(){
    ...
    $xml_files = get_files_list_from_folder($path_xml); //доработка
    $image_files = get_files_list_from_folder($path_images); //доработка
    ...
    if(count($image_files) > 0){
        ...
        foreach($image_files as $key=>$image){
            ...
            $path_to_file = $path_images . $image; //доработка
...
function send_json_post_request_to_opencart_api($json, $filename){
    ...
    add_to_log('Отправлен json с товарами в opencart api ' . $filename); //доработка
    ...
    curl_close($ch); // доработка, перенес

    add_to_log("Код ответа сервера opencart " . $response_code); //Доработка
    add_to_log($opencart_response_json); //Доработка
...
function send_image_post_request_to_opencart_api($file_name, $file_path){
    ...
    add_to_log('Отправлена картинка в opencart api ' . $file_path);  //доработка
...
function clear_id($id){
    ...
    //gdhf-121212-sdfsdffs#sadssfsf-123123123-adfsdf
...
define("PATH_TO_XML", "import_1c_xml_test");
define("PATH_TO_IMAGE", "import_1c_image_test");
define("OPENCART_URL_RECEIVER", "http://s5.1c-shops.ru/index.php?route=api/exchange1c/load_data");
...
if ($_GET['mode'] || $_POST['mode']){
    ...
    if ($message){
        add_to_log('Обмен прошел успешно');
    }
